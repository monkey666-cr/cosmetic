...
class RiveParser:
    ...
    def get_product_id(element: etree.Element):
        try:
            product_id = element.xpath('//input[@name="productCodePost"]')[0].get("value")
    ...
    def get_page_low_price(element: etree.Element):
        try:
            price = element.xpath('//*[@itemprop="lowPrice"] | //*[contains(@class, "discount-gold_price")]')[0].get("content")
            if not price:
    ...
    def get_product_status(element: etree.Element):
        try:
            class_attr = element.xpath('//div[@class="links"]//a')[0].get("class")
